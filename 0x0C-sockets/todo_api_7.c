 ...
 * @resp_type: determines what to check, get and delete share the same macro
 *
 * Return: non-negative value potentially specifying get/delete id on success,
 * -1 on error
 */
...
void parse_req(char *buffer, int client_fd, todo_info_t *td_info)
{
	...
	else if (strncmp(buffer, DELETE, DELETE_LEN) == 0)
	{
		del_resp(client_fd, td_info, req_ret);
	}
